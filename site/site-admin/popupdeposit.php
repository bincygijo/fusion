...
$date		=	date('Y-m-d');
 ...
 //print_r($arrUser);
  ...
  $reason		=	'Payment paid as deposit';
 //echo $account_id;
if(isset($_POST['add']))
{
...
$qry		=	mysql_query("insert into new_history(client_id,history_staffname,history_date,history_school,history_summary,Transation_type,amount)values($client_Id,'$staff_name','$date','$school_name','$reason','$transId','$amount')");
//echo $otal;
//$total		=	


$msg			=	$objP->modifyAccounts($dateRece,$transId,$note,$total,$client_Id,$account_id);
...
<table width="540" border="0" cellspacing="0" cellpadding="0" class="border_box">
  ...
  <tr>
	...
	<?php if(!empty($arrTrans))
		...
		{ ?>
		<option value="<?=$value['name']?>" <?php if($trans_type==$value['trans_id']) echo 'selected';?> >
		 <?=$value['name']?></option>
		<?php }
		}?>
        ...
        <tr>
          ...
          <td>&nbsp;</td>
          <td height="35" valign="bottom"><input type="text" name="amountrece" id="amountrece" value=""  class="field"/></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>

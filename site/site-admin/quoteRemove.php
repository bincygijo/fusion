...
$objH 			=	new history();
    ...
    $cont_id 				=  $_GET['id'];
		...
		//for($i = 0; $i < count($client_id)-1; $i++){
			...
			$contact_id		= 	$objU->getQuote($cont_id);
		//	print_r($contact_id);
			...
			$no_books		=	$contact_id['nobooks'];
			
			$nopage			=	$contact_id['nopages'];
			$budget			=	$contact_id['budget'];
			$comments 		=	$contact_id['comments'];
			...
			$qry		=	$objU->addregister_quote($cont_id,$name,$school,$email,$no_books,$nopage,$budget,$comments,$details,$rdate);
	
	
			$msg			= 	$objU->deleteQuote($cont_id);	
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#0d76aa">
                          <tr>
                            ...
                            <td class="head">Add Quote(Contacted)</td>

	...
	$.fn.scrollShow = function( settings ){
		...
		return this.each(function(){
			...
			function sequential( event ){
				...
				var setnew;
				var position		=	active +event.data.dir;
				var imgCount 		= document.getElementById("count_id").value;
				//alert("imgCount"+imgCount+"===position"+position+"active"+active);
				if(position == imgCount){
					setnew = 0;
				}else if(event.data.dir==-1 && position==-1 && active==0){
					setnew = imgCount-1;
					//alert("here"+setnew);
				}else if(event.data.dir==-1 && (position<active)){
					setnew = position;
				}else if(event.data.dir==1){
					setnew = active +1;	
				}
				
				//alert("imgCount"+imgCount+"===position"+position+"active"+active);
				//alert("setnew"+setnew);
				//if(document.getElementById("photo_id_h"+active))
				
				//	{
						
				<!--document.getElementById("rateType_item").innerHTML='<span class="quotes">&ldquo;</span>'+document.getElementById("photo_id_h"+setnew).value+'<span class="quotes">&rdquo;</span>';-->
				//	}
								
					event.data.pos = active + event.data.dir;
				//alert(event.data.dir);	
